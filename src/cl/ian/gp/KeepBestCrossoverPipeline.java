 ...
 */
public class KeepBestCrossoverPipeline extends CrossoverPipeline {
...

    {
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
        {
            ...
            Individual[] individuals = new Individual[] { j1, j2,parents[0],parents[1]};
            Arrays.sort(individuals);

            // add the individuals to the population
            inds[q] = individuals[0];
            q++;
            if (q<n+start && !tossSecondParent)
            {
                inds[q] = individuals[1];
                q++;
            }
        }
        return n;
