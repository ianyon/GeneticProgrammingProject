 ...
 */
public class KeepBestMutateOneNodePipeline extends GPBreedingPipeline {
    private static final long serialVersionUID = 1;

    public static final String P_KEEPBESTMUTATEONENODEPIPELINE = "keep-best-mutate-one-node";
    ...
    public Object clone() {
        KeepBestMutateOneNodePipeline c = (KeepBestMutateOneNodePipeline) (super.clone());
    ...
    /**
     * Returns a node which is swap-compatible with returntype, and whose arguments are swap-compatible with the
     * current children of original.  You need to clone this node.
     */
    protected GPNode pickCompatibleNode(final GPNode original, final GPFunctionSet set, final EvolutionState state,
    ...
    public int produce(final int min,
        ...
        for (int q = start; q < n + start; q++) {
            ...
            if (tree != TREE_UNFIXED && (tree < 0 || tree >= i.trees.length))
                // uh oh
                state.output.fatal("KeepBestMutateOneNodePipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
